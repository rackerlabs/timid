...
if [ ${venv} = yes ]; then
    ...
    echo "Installing timid setup in the virtual environment"
    python ${dir}/setup.py install
    if [ $? -ne 0 ]; then
        echo "Failed to install timid setup in virtual environment" >&2
        exit 1
    fi
...
if [ ${pep8} != only ]; then
	...
	options="${options} --cover-package=timid"
